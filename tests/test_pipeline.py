...
class TestPipeline:
    @staticmethod
    def check_loaded_data(cfg: Config, trainer_tester: Union[Trainer, Tester], data: xr.Dataset, is_train: bool = True):
        if is_train:
        ...
        if cfg.static_inputs is not None:
            # check static size
            assert dataloader.dataset.x_s[pixel].numpy().shape == (len(cfg.static_inputs),)
